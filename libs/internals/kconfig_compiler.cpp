...
static void parseArgs(const QStringList &args, QString &directory, QString &file1, QString &file2)
{
    ...
    for (int i = 1; i < args.count(); ++i)
    {
        if (args.at(i) == "-d" ||  args.at(i) == "--directory")
        {
            ...
            if (args.at(i).startsWith(QLatin1String("-d")))
            {
                ...
                if (args.at(i) == "--help" || args.at(i) == "-h")
                {
                    ...
                    if (args.at(i) == "--license" || args.at(i) == "-L")
                    {
                              ...
                              QString &code)
{
    if (type == "String" && !defaultValue.isEmpty())
    {
        ...
        if (type == "Path" && !defaultValue.isEmpty())
        {
            defaultValue = literalString(defaultValue);

        }
        else
            if ((type == "StringList" || type == "PathList") && !defaultValue.isEmpty())
            {
                ...
                if (type == "Color" && !defaultValue.isEmpty())
                {
...
int main(int argc, char **argv)
{
    ...
    if (setUserTexts)
        cpp << "#include <klocale.h>" << endl << endl;
    ...
    for (itEntry = entries.constBegin(); itEntry != entries.constEnd(); ++itEntry)
    {
        ...
        if (t == "Enum")
        {
            ...
            if (chlist.isEmpty())
            {
                ...
                // switch the value and write the choice name as string instead of the enum's int value
            {
