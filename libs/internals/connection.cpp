...
Connection::Type Connection::typeFromString(const QString &typeString)
{
    ...
    if (typeString == QLatin1String("802-3-ethernet"))
    {
        ...
        if (typeString == QLatin1String("802-11-wireless"))
        {
            type = Wireless;
        }
        else
            if (typeString == QLatin1String("gsm"))
            {
                type = Gsm;
            }
            else
                if (typeString == QLatin1String("cdma"))
                {
                    type = Cdma;
                }
                else
                    if (typeString == QLatin1String("bluetooth"))
                    {
                        type = Bluetooth;
                    }
                    else
                        if (typeString == QLatin1String("vpn"))
                        {
                            type = Vpn;
                        }
                        else
                            if (typeString == QLatin1String("pppoe"))
                            {
                                type = Pppoe;
                            }
    return type;
...
Connection::Type Connection::typeFromSolidType(const NetworkManager::Device::Ptr &iface)
{
    switch (iface->type())
    {
        ...
        case NetworkManager::Device::Bluetooth:
            return Knm::Connection::Bluetooth;
...
{
    addToPermissions(Environment::userName(), QString());
    init(bt_cap);
}
...
void Connection::setType(Connection::Type type, NMBluetoothCapabilities bt_cap)
{
    ...
    qDebug() << "Connection type is set as " << typeAsString(type) << ". Settings of the connection removed since its type has been changed.";
}
