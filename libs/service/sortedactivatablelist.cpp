...
//X         qDebug() << s1 << "and" << s2 << "are on the same device, comparing activatable type";
    ...
    if (i == 0) {
        ...
        if (firstIc && secondIc) {
//X             qDebug() << s1 << "and" << s2 << "have the same type, comparing activation state" << firstIc->activationState() << secondIc->activationState();
...
//X                     qDebug() << s1 << "and" << s2 << "have the same state, comparing name";
...
//X                     qDebug() << s1 << "and" << s2 << "have the same state, comparing signal strength";
     ...
     //               qDebug() << s1 << "and" << s2 << "are both wireless networks, comparing SSID";
                    i = compareSsid(firstW, secondW);
                } else {
//X                     qDebug() << s1 << "and" << s2 << "are both wireless networks, comparing signal strength";
    ...
    // debug
    if ( i < 0 )
        qDebug() << s1 << "is before"  << s2;
    else if (i > 0)
        qDebug() << s2 << "is before"  << s1;
    else {
