...
NMVariantMapMap NMDBusSecretAgent::GetSecrets(const NMVariantMapMap &connection, const QDBusObjectPath &connection_path, const QString &setting_name, const QStringList &hints, uint flags)
{
    ...
    //qDebug() << connection;
    qDebug() << setting_name << flags << hints;
...
void NMDBusSecretAgent::SaveSecrets(const NMVariantMapMap &connection, const QDBusObjectPath &connection_path)
{
    ...
    if (d->secretsProvider)
    {
        ...
        qDebug() << "Secrets are being saved for connection " << con->uuid();
        d->secretsProvider->saveSecrets(con);
    }
    else
    {
        qDebug() << "Secrets for" << con->uuid() << "not save because there is no secretsProvider registered.";
    }
}
...
void NMDBusSecretAgent::DeleteSecrets(const NMVariantMapMap &connection, const QDBusObjectPath &connection_path)
{
    ...
    if (d->secretsProvider)
    {
        qDebug() << "Deleting secrets for connection " << con->uuid();
        d->secretsProvider->deleteSecrets(con);
    }
    else
    {
        qDebug() << "Secrets for" << con->uuid() << "not deleted because there is no d->secretsProvider registered.";
    }
}
...
void NMDBusSecretAgent::secretsReady(Knm::Connection *con, const QString &name, bool failed, bool needsSaving)
{
    ...
    {
        qWarning() << "Unexpected secrets arrived for" << pair.first << name;
    }
}
