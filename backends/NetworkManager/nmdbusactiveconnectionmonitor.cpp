    ...
    : m_activeConnectionIface(activeConnectionIface), m_interfaceConnection(interfaceConnection)
{
    ...
    qDebug() << "default:" << m_activeConnectionIface->getDefault() << "state:" << m_activeConnectionIface->state();
...
void NMDBusActiveConnectionProxy::setState(uint nmState)
{
    ...
    switch (nmState)
    {
        ...
        default
                :
            qDebug() << "Unhandled activation state" << nmState;
    }
    qDebug() << "state:" << aState;
...
void NMDBusActiveConnectionMonitor::activeConnectionListChanged()
{
    ...
    while (i.hasNext())
    {
        ...
        if (!currentActiveConnections.contains(i.key()))
        {
            ...
            qDebug() << "removing stale active connection" << i.key();
            i.remove();
            delete stale;
        }
    }
    ...
    foreach (const QString & activeConnectionPath, currentActiveConnections)
    {
        ...
        {
            qDebug() << "Adding active connection interface for " << activeConnectionPath;
            ...
            qDebug() << "Connection active at" << active->connection().path() << (active->vpn() ? "is" : "is not") << "a VPN connection";
        }
    }
}
...
void NMDBusActiveConnectionMonitor::handleRemove(Knm::Activatable *removed)
{
    ...
    while (i.hasNext())
    {
        ...
        {
            i.remove();
            qDebug() << "removing active connection because its connection was removed";
            delete candidate;
        }
    }
}
