...
void NMDBusSettingsConnectionProvider::initConnections()
{
    ...
    {
        qDebug() << "Error in ListConnections() D-Bus call:" << reply.error();
    }
}
...
void NMDBusSettingsConnectionProvider::initialiseAndRegisterRemoteConnection(const QString &path)
{
    ...
    {
        qDebug() << "Service" << d->iface->service() << "is reporting the same connection more than once!";
    }
    else
    {
...
void NMDBusSettingsConnectionProvider::handleAdd(Knm::Activatable *added)
{
    ...
    if (interfaceConnection)
    {
        ...
        {
            qDebug() << "tagging InterfaceConnection " << interfaceConnection->connectionName() << "from" << d->serviceName << d->uuidToPath[interfaceConnection->connectionUuid()];
...
void NMDBusSettingsConnectionProvider::interfaceConnectionActivated()
{
    ...
    if (ic)
    {
        ...
        if (vpn)
        {
            ...
            qDebug() << "active" << activeConnPath << "device" << deviceToActivateOn;
...
void NMDBusSettingsConnectionProvider::interfaceConnectionDeactivated()
{
    ...
    if (iface)
    {
        ...
        // VPN connections do have NetworkInterface objects.
    {
...
void NMDBusSettingsConnectionProvider::updateConnection(const QString &uuid, Knm::Connection *newConnection)
{
    ...
    if (d->uuidToPath.contains(QUuid(uuid)))
    {
        ...
        {
            qWarning() << "Connection could not found!" << uuid << objPath;
            return;
        }

        RemoteConnection *remote = d->connections.value(uuid);
        qDebug() << "Updating connection " << remote->id() << objPath;
    ...
    qWarning() << "Connection could not found!" << uuid;
}
...
void NMDBusSettingsConnectionProvider::addConnection(Knm::Connection *newConnection)
{
    ...
    ConnectionDbus converter(newConnection);
    NMVariantMapMap map = converter.toDbusMap();
    qDebug() << "Adding connection " << newConnection->name() << newConnection->uuid().toString();
    // WARNING: this debug message print secrets, do not commit it uncommented.
    //qDebug() << "Here is the map: " << map;

    if (newConnection && newConnection->name().isEmpty())
        qWarning() << "Trying to add connection without a name!";
...
void NMDBusSettingsConnectionProvider::onConnectionAddArrived(QDBusPendingCallWatcher *watcher)
{
    ...
    if (!reply.isValid())
    {
        qWarning() << "Adding connection failed:" << reply.error().message();
    ...
    else
    {
        ...
        qDebug() << "Connection added successfully: " << objPath.path() << uuid;
    }
...
bool NMDBusSettingsConnectionProvider::getConnectionSecrets(Knm::Connection *con)
{
    ...
    {
        qWarning() << "Secrets requested but connection not found!";
        return false;
    }
    ...
    qDebug() << "Getting connection secrets for " << uuid.toString();

    QStringList secretSettings = con->hasPersistentSecretsSettings();

    qDebug() << "Settings containing secret values are " << secretSettings;

    if (secretSettings.count() != 1)
        qWarning() << "This connection has more than 1 secret setting, not supported yet :/";
...
void NMDBusSettingsConnectionProvider::onConnectionSecretsArrived(QDBusPendingCallWatcher *watcher)
{
    ...
    if (reply.isValid())
    {
        ...
        //qDebug() << "Got secrets, yay! " << set;
        qDebug() << "Got secrets, yay! ";
        ...
        {
            qWarning() << "Connection not found!" << watcher->property("connection").toString();
        }
    }
    else
    {
        qWarning() << "Secret fetching failed:" << reply.error().message();
...
void NMDBusSettingsConnectionProvider::removeConnection(const QString &uuid)
{
    ...
    if (d->uuidToPath.contains(QUuid(uuid)))
    {
        ...
        qDebug() << "Removing connection " << remote->id() << uuid;
        remote->Delete();
