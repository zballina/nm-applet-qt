...
NMVariantMapMap ConnectionDbus::toDbusMap()
{
    ...
    if (!m_connection->permissions().isEmpty())
    {
        ...
        for (i = permissions.constBegin(); i != permissions.constEnd(); ++i)
        {
            permissionsDbus.append(QLatin1String("user:") + i.key() + ':' + i.value());
        }
...
void ConnectionDbus::fromDbusSecretsMap(const NMVariantMapMap &secrets)
{
    ...
    //qDebug() << "Original settings:" << origs;
    ...
    for (i = secrets.constBegin(); i != secrets.constEnd(); ++i)
    {
        ...
        if (secret.isEmpty())
        {
            qDebug() << "Empty secret setting found '" << i.key() << "', skipping...";
            continue;
        }
        ...
        if (origs.contains(i.key()))
        {
            ...
            //qDebug() << "Uniting setting " << secret.keys() << " with values " << secret.values();
