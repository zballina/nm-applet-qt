...
void ConnectionsListModel::showMoreClicked()
{
    ...
    QSettings config;
    config.beginGroup("General");
    config.setValue(QLatin1String("ShowMoreConnections"), m_oldShowMoreChecked);
    config.sync();
    config.endGroup();
}
...
void ConnectionsListModel::showMore(bool show)
{
    ...
    updateConnectionsList();
    qDebug() << "Show == " << show << " wicCount == " << wicCount;
}
...
void ConnectionsListModel::removeHiddenItem()
{
    if (hiddenInserted)
    {
        removeRow(0);
        hiddenInserted = false;
        qDebug() << "Will remove hidden item";
    }
}
...
{
    Q_UNUSED(parent);
    if (row < 0 || (row + count) >= connections.size())
        return false;
...
QModelIndex ConnectionsListModel::indexFromItem(const ConnectionItem *item) const
{
    if (item)
    {
        ...
        {
            if (connections.at(row)->equals(item))
                return index(row);
        }
    }
    return QModelIndex();
...
void ConnectionsListModel::setFilter(QVariant tabName)
{
    if (tabName == tr("Connections"))
    {
        ...
        if (tabName == tr("VPN"))
        {
            ...
            if (tabName == tr("Shared Connections"))
            {
                currentFilter &= ~VpnConnections;
                currentFilter &= ~SharedConnections;
                currentFilter |= SharedConnections;
...
void ConnectionsListModel::connectToHiddenNetwork(QVariant ssidParam)
{
    ...
    qDebug() << "invoking networkmanagement_configshell" << args;
}
