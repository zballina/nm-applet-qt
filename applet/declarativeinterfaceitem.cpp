    ...
    m_starting(true)
{
    ...
    if (m_iface)
    {
        ...
        if (m_iface.data()->type() == NetworkManager::Device::Ethernet)
        {
            ...
            if (m_iface.data()->type() == NetworkManager::Device::Wifi)
            {
                m_type = "wifi";
                setConnectionInfo();
            }
            else
                if (m_iface.data()->type() == NetworkManager::Device::Modem)
                {
                    m_type = "modem";
                }
                else
                    if (m_iface.data()->type() == NetworkManager::Device::Bluetooth)
                    {
                        m_type = "bluetooth";
                    }
        m_starting = false;
...
void DeclarativeInterfaceItem::handleHasDefaultRouteChanged(bool changed)
{
    ...
    setInterfaceIcon();
    qDebug() << "Default Route changed!!" << changed;
    emit itemChanged();
}
    ...
    if (m_type == "vpn")
    {
        ...
        if (m_currentConnection)
        {
            ...
            if (m_currentConnection->activationState() == Knm::InterfaceConnection::Activated)
            {
                ...
                if (m_currentConnection->activationState() == Knm::InterfaceConnection::Activating)
                {
        ...
        {
            //qDebug() << m_vpnActivatables.count() << "VPN connections have become available!";
            m_visible = true;
        }
        else
        {
            //qDebug() << "hidding VPN widget:" << m_vpnActivatables.count();
            m_visible = false;
        }
