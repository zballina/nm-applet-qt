...
{
    qDebug() << "load traffic plotter with index " << index;
    if (index >= 0 && index < interfaces.size())
    {
        emit updateTraffic(interfaces[index]);
        qDebug() << "emitted update traffic plotter signal";
    }
}
...
{
    Q_UNUSED(parent);
    if (row < 0 || (row + count) >= interfaces.size())
        return false;
...
QModelIndex InterfacesListModel::indexFromItem(const DeclarativeInterfaceItem *item) const
{
    if (item)
    {
        ...
        {
            if (interfaces.at(row)->equals(item))
                return index(row);
        }
    }
    return QModelIndex();
